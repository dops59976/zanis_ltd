      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((testimonial, idx) => (
            <div
              key={idx}
              className="p-8 rounded-2xl bg-white border border-gray-200 hover:border-gray-400 card-hover group"
            >
              <div className="flex gap-1 mb-4">
                {[...Array(5)].map((_, i) => (
                  <span key={i} className="text-black transform group-hover:scale-125 transition" style={{transitionDelay: `${i * 50}ms`}}>
                    ★
                  </span>
                ))}
              </div>
              ...
              <div>
                <p className="font-semibold text-black group-hover:text-gray-800 transition">
                  {testimonial.author}
                </p>
